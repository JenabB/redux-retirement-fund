//expenditure input
export const GET_EPY = "GET_EPY"; //expenditure per year
export const GET_EPM = "GET_EPM"; //expenditure per month

//expense retirement
export const GET_CA = "GET_CA"; //current age
export const GET_AAR = "GET_AAR"; //age at retirement
export const GET_AI = "GET_AI"; //annual inflation

//result
export const GET_TAE = "GET_TAE"; //total amount of expenditure
export const GET_RAOR = "GET_RAOR"; //required amount of retirement

export const GET_ROP = "GET_ROP";
export const GET_CPF = "GET_CPF";
export const GET_AIEM = "GET_AIEM";
export const GET_AIEA = "GET_AIEA";
